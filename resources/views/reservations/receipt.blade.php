<x-admin-layout>
    <div class="mb-4">
        <a href="{{ route('reservations.index') }}" class="text-sm bg-slate-600 text-white p-2 rounded-lg flex items-center">
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
        ...
        <div class="bg-white p-6 rounded-lg shadow-md w-1/2 mx-auto mt-4">
            <div class="text-gray-900 border-b border-gray-300 pb-4 mb-4">
                ...
                <p><strong>Check-in Date:</strong> {{ \Carbon\Carbon::parse($reservation->check_in)->format('F j, Y g:i A') }}</p>
                <p><strong>Check-out Date:</strong> {{ \Carbon\Carbon::parse($reservation->check_out)->format('F j, Y g:i A') }}</p>
                <p class="text-lg font-bold"><strong>Total Amount:</strong> ₱{{ number_format($reservation->total_amount, 2) }}</p>
            </div>
            ...
            <div>
                ...
                <ul class="list-disc list-inside text-gray-700">
                    @foreach ($reservation->rooms as $room)
                        <li>{{ $room->room_number }} - {{ $room->room_type }} ₱{{ number_format($room->price, 2) }} - {{ $room->stay_type }}</li>
                    @endforeach
                </ul>
            </div>
        </div>
