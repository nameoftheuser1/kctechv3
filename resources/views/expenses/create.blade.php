<x-admin-layout>
    <div class="container mx-auto px-4 py-8 w-full sm:w-1/2 border rounded-lg bg-white mt-0 sm:mt-10">
        <a href="{{ route('expenses.index') }}" class="text-blue-500 text-sm underline">&larr; back to expense list</a>
        <h1 class="text-3xl font-bold text-slate-700 mt-4">Create New Expense</h1>
        <p class="text-sm text-slate-500 mb-6">Add expense details</p>
        <form action="{{ route('expenses.store') }}" method="POST" class="flex flex-col w-full justify-center">
            @csrf
            <div class="mb-4 w-full">
                <label for="expense_description" class="block text-gray-700 font-bold mb-2 text-sm">Expense Description</label>
